...
document.addEventListener('DOMContentLoaded', () => {
  ...
  document.querySelectorAll('.video-thumb').forEach((v) => {
    v.addEventListener('click', () => {
      const videoUrl = v.getAttribute('data-video');
      videoWrapper.innerHTML = `<iframe src="${videoUrl}?autoplay=1" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture" allowfullscreen></iframe>`;
  ...
  if (qCheck) {
    qCheck.addEventListener('click', () => {
      ...
      if (
        ok.includes(clean) ||
        (clean.includes('mi') && clean.includes('unos'))
      ) {
        res.textContent = '✅ Bien: "mi" y "unos" son determinantes.';
        res.style.color = 'green';
      } else {
        res.textContent =
          '❌ Intenta de nuevo. Busca palabras que acompañen al sustantivo.';
  ...
  if (jCheck) {
    jCheck.addEventListener('click', () => {
      ...
      if (a.includes('aquellos') && a.includes('dos')) {
        r.textContent =
          '✅ Correcto: "aquellos" (demostrativo) y "dos" (numeral).';
        r.style.color = 'green';
      } else {
        r.textContent =
          '❌ Faltan determinantes. Intenta mencionar "aquellos" y "dos".';
  ...
  if (pCheck) {
    pCheck.addEventListener('click', () => {
      ...
      if (hasDem && hasPos && hasNum) {
        r.textContent =
          '✅ Excelente: tu oración contiene los tres tipos solicitados.';
        r.style.color = 'green';
      } else {
        r.textContent =
          '❌ Tu oración aún no contiene los tres tipos (demostrativo, posesivo, numeral).';
        r.style.color = 'crimson';
      }
    });
  }
